...
class NBackGame:
    def __init__(self, root):
        ...
        self.letter_interval = 600  # milliseconds
        ...
        self.press_label = tk.Label(root, text="", font=("Helvetica", 40), bg="white", fg="blue")
        self.press_label.place(relx=0.5, rely=0.65, anchor="center")

        self.ui_frame = tk.Frame(root, bg="white")
        self.ui_frame.pack(pady=20)

        self.start_button = tk.Button(self.ui_frame, text="Start Game", command=self.start_game, font=("Helvetica", 16))
        self.start_button.grid(row=0, column=0, columnspan=2, pady=(10, 20))

        self.uid_label = tk.Label(self.ui_frame, text="User ID (for log name):", font=("Helvetica", 16), bg="white", fg="black")
        self.uid_label.grid(row=1, column=0, sticky="e", padx=10, pady=5)
        self.uid_entry = tk.Entry(self.ui_frame, font=("Helvetica", 16), width=20)
        self.uid_entry.grid(row=1, column=1, sticky="w", pady=5)

        self.n_label = tk.Label(self.ui_frame, text="N-Back Level (e.g., 2):", font=("Helvetica", 16), bg="white", fg="black")
        self.n_label.grid(row=2, column=0, sticky="e", padx=10, pady=5)
        self.n_entry = tk.Entry(self.ui_frame, font=("Helvetica", 16), width=20)
        self.n_entry.grid(row=2, column=1, sticky="w", pady=5)

        self.case_sensitive_var = tk.BooleanVar(value=False)
        self.case_checkbox = tk.Checkbutton(
            self.ui_frame,
            text="Case-sensitive Match (A ≠ a)",
            variable=self.case_sensitive_var,
            font=("Helvetica", 14),
            bg="black"
        )
        self.case_checkbox.grid(row=3, column=0, columnspan=2, pady=(10, 10))
    ...
    def start_game(self):
        ...
        self.ui_frame.pack_forget()  # Hide input UI
        ...
        for i in range(self.total_rounds):
            if i in match_indices:
                ...
                while True:
                    ...
                    if i < self.n or not self.is_match(letter, self.sequence[i - self.n]):
    ...
    def show_next_letter(self):
        ...
        self.press_label.config(text="")  # Clear "Press!" message here
    ...
    def handle_space(self, event):
        ...
        match_expected = (
            ...
            self.is_match(self.sequence[self.current_index], self.sequence[self.current_index - self.n])
        )

        self.responses.append((self.current_index, match_expected, True))
        self.response_times.append(response_time)

        # Show feedback and "Press!"
        self.press_label.config(text="Press!")
    ...
    def end_game(self):
        ...
        for i in range(self.n, self.total_rounds):
            if self.is_match(self.sequence[i], self.sequence[i - self.n]):
                total_matches += 1
        ...
        misses = total_matches - correct_hits
        ...
        self.save_log_csv(accuracy, avg_response_time, correct_hits, misses, total_matches)

    def save_log_csv(self, accuracy, avg_rt, hits, misses, total):
        log_exists = os.path.exists("nback_log.csv")
        with open("nback_log.csv", "a", newline='') as csvfile:
            writer = csv.writer(csvfile)
            if not log_exists:
                writer.writerow([
                    "user_id", "n_back", "accuracy", "average_response_time",
                    "hits", "misses", "total_matches", "case_sensitive"
                ])
            writer.writerow([
                self.user_id, self.n, f"{accuracy:.2f}", f"{avg_rt:.3f}",
                hits, misses, total,
                "Yes" if self.case_sensitive_var.get() else "No"
            ])
